...
def load_crime_data_df(spark_session):
    return cleanup_crime_data(spark_session \
        .read \
        .csv("/home/data/CrimeData/", header=True, inferSchema=True))
    
    
def load_crime_data_from_single_parquet_file(spark_session):
    return cleanup_crime_data(spark_session \
        .read \
        .parquet("/home/data/crime_data.parquet", header=True, inferSchema=True))
...
def load_census_blocks_df(spark_session):
    ...
    census_blocks_df = sedona \
        ...
        .load("/home/data/2010_Census_Blocks.geojson") \
...
def load_income_df(spark_session):
    income_df = spark_session \
        .read \
        .csv("/home/data/LA_income_2015.csv", header=True, inferSchema=True)
    income_df = income_df \
        .withColumn(
...
def load_re_codes_df(spark_session):
    return spark_session \
        .read \
        .csv("/home/data/RE_codes.csv", header=True, inferSchema=True)
